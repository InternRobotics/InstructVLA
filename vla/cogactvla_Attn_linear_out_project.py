...
class ActionTransformer(nn.Module):
    def __init__(self, token_size, past_action_window_size, future_action_window_size, action_dim, num_layers=12, nhead=8, dim_feedforward=None, dropout=0.1):
        ...
        self.future_queries = nn.Parameter(torch.randn(future_action_window_size + 1, token_size))  # Learnable future queries
        self.positional_encoding = nn.Parameter(torch.randn(past_action_window_size + 1, token_size))  # Learnable positional encoding
        ...
        self.mlp = nn.Sequential(
            nn.Linear(token_size, action_dim)
        )

    def forward(self, past_actions):
        """
        past_actions: (batch_size, past_action_window_size + 1, token_size)
        """
        batch_size = past_actions.shape[0]
        
        # Apply embedding and add positional encoding
        past_actions += self.positional_encoding  # Add learnable positional encoding

        # Expand future queries for batch
        future_queries = self.future_queries.unsqueeze(0).expand(batch_size, -1, -1)  # (batch, future_seq_len, token_size)

        # Concatenate past actions and future queries
        input_seq = torch.cat([past_actions, future_queries], dim=1)  # (batch, total_seq_len, token_size)

        # Transformer requires (seq_len, batch, feature)
        input_seq = input_seq.permute(1, 0, 2)  # (seq_len, batch, token_size)

        # Apply self-attention
        encoded_seq = self.transformer_encoder(input_seq)  # (seq_len, batch, token_size)
        
        # Extract the future part and pass through MLP
        future_pred = encoded_seq[-(self.future_action_window_size + 1):]  # (future_seq_len, batch, token_size)
        output = self.mlp(future_pred)  # (future_seq_len, batch, action_dim)

        return output.permute(1, 0, 2)  # (batch, future_seq_len, action_dim)
...
class CogACT(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        indices_for_past = torch.cat([torch.arange(i*(self.past_action_window_size+1), i*(self.past_action_window_size+1)+self.past_action_window_size) for i in range(per_device_batch_size)])  # 选取 B*N 个索引
        ...
        with torch.no_grad():
            if pixel_values is not None:
                ...
                for k, v in pixel_values.items(): 
                    pixel_values_past[k] = v[indices_for_past]
            else:
                pixel_values_past = None
            output_for_past: CausalLMOutputWithPast = self.vlm(
                ...
                pixel_values=pixel_values_past,
        ...
        if pixel_values is not None:
            pixel_values_now = {}
            for k, v in pixel_values.items():
                pixel_values_now[k] = v[indices_for_now]
        else:
            pixel_values_now = None
        ...
        pred = self.action_model(cognition_features)
    ...
    def predict_action(
        ...
        **kwargs: str
        ...
        samples = self.action_model(cognition_features)
