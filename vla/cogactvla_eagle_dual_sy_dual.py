...
class ActionWorldModel(nn.Module):
    def __init__(self, 
        ...
        apply_activation_checkpointing(self.VideoGPT, checkpoint_wrapper_fn=non_reentrant_wrapper, check_fn=check_fn)
    ...
    def forward(
        ...
        **kwargs,
        ...
        output: CausalLMOutputWithPast = self.vlm(
            ...
            # labels=labels,
            # inputs_embeds=inputs_embeds,
            image_flags=torch.ones((first_past_indices.shape[0],1)).to( device=input_ids.device),
            past_key_values=past_key_values,
            use_cache=use_cache,
            output_attentions=output_attentions,
            output_hidden_states=output_hidden_states,
        ...
        pred = self.action_model( cognition_features = repeated_cognition_features[sampled_indices],
                                pixel_values = dict(dino = system2_pixel_values[sampled_indices]),
                                indices_for_now = None,
                                )
        
        loss = F.mse_loss(pred, actions_future[sampled_indices])
        return loss, None
