#!/bin/bash
#SBATCH --job-name=MergeSYS12        # name
#SBATCH -p mozi-S1
#SBATCH -N 5                    # nodes
...
srun --jobid $SLURM_JOBID bash -c 'python -m torch.distributed.run \
 ...
 scripts/train_eagle_dual.py \
  --vla.base_vlm "/mnt/petrelfs/yangshuai1/yangshuai1/share_mllm/Eagle2-2B" \
  --vla.type prism-qwen25-dinosiglip-224px+0_5b \
  --pretrained_checkpoint /mnt/petrelfs/yangshuai1/rep/cogact_with_history/outputs/head_balation/sys12_dual_16_GPU_fractal_only--image_aug/checkpoints/step-019500-epoch-01-loss=0.0481.pt \
  --vla.data_mix RT_only \
  --vla.expected_world_size 40 \
  --vla.global_batch_size 560 \
  --vla.per_device_batch_size 14 \
  ...
  --run_id sys12_dual_16_GPU_fractal_only \
  --image_aug True \
