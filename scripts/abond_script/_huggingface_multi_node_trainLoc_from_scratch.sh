...
srun --jobid $SLURM_JOBID bash -c 'python -m torch.distributed.run \
 ...
 scripts/train.py \
  ...
  --data_root_dir "/mnt/hwfile/OpenRobotLab/robot_data/OXE_openvla" \
  --run_root_dir ./outputs/HIS_ditb_8_lr2e-5_b16_fa15_pa0_shuffle_oxe \
  --run_id HIS_prism-dinosiglip-224px+oxe+diffusion+n8+b16+x42 \
  --image_aug True \
  --wandb_project cogact \
  --wandb_entity lihaohn \
  --save_interval 5000 \
  --repeated_diffusion_steps 6 \
  --future_action_window_size 15 \
  --past_action_window_size 7 \
