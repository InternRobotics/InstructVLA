gpu_id=$2
policy_model=cogact
ckpt_path=$1 # CogACT/CogACT-Large CogACT/CogACT-Small
...
CUDA_VISIBLE_DEVICES=${gpu_id} python simpler_env/main_inference_server.py --host $AVAILABLE_HOST --port $AVAILABLE_PORT --policy-model cogact --ckpt-path $1 --policy-setup widowx_bridge --env-name None &
SERVER_PID=$!
